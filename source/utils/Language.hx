...
class Language {
    ...
    public static function get_list():Array<String> {
        ...
        for (path in Paths.readDirectory('assets/data/lang/')) {
            ...
            var cur_lang:String = path.split("/").pop().replace(".json", "");
    ...
    public static function load(?new_lang:String):Void {
        ...
        current = Paths.exists(Paths.getPath('data/lang/${new_lang}.json', TEXT)) ? new_lang : "English";

        var new_data:Map<String, Dynamic> = [];
        for (cur_path in Paths.readFile('assets/data/lang/${current}.json')) {
